...
PointLightShaderClass::PointLightShaderClass()
{
	...
	pixelShader = PixelShader::loadFromFile(L"DeferredPointLightPixelShader.hlsl", &light_buffer_desc);
}

void PointLightShaderClass::prepare(GameObject * object)
{
	PointLight* lightComponent = object->getComponent<PointLight>();
	...
	lightBufferValues.lightPosition = object->getTransform()->getWorldPositionVector();
	...
	lightBufferValues.shininess = 5.0f;
	lightBufferValues.viewPosition = object->getScene()->getActiveCamera()->getGameObject()->getTransform()->getWorldPositionVector();
